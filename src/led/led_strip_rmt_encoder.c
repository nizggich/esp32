    ...
    switch (led_encoder->state)
    {
        case 0: // send RGB data
            ...
            if (session_state & RMT_ENCODING_MEM_FULL)
            {
                ...
                goto out; // yield if there's no free space for encoding
                          // artifacts
            }
        // fall-through
                                    ...
                                    rmt_encoder_handle_t *ret_encoder)
{
    ...
    ESP_GOTO_ON_FALSE(config->led_model < LED_MODEL_INVALID, ESP_ERR_INVALID_ARG, err, TAG,
                      "invalid led model");
    ...
    uint32_t reset_ticks = config->resolution / 1000000 * 280 /
